...
public class CompassTeleport {
    ...
    public static boolean hasTeleportingCompass() {
        ...
        return Tomrum.CONFIG.compassTeleport && player.capabilities.isCreativeMode
            && player.getHeldItem() != null
            && player.getHeldItem()
                .getItem() == Items.compass;
    }
    ...
    private void teleportToSafeLocation(final EntityClientPlayerMP player, final float x, final float y,
        final float z) {
    ...
    public MovingObjectPosition rayTraceBlocks(final World world, int steps, final Vec3 from, final Vec3 to) {
        if (Double.isNaN(from.xCoord) || Double.isNaN(from.yCoord)
            || Double.isNaN(from.zCoord)
            || Double.isNaN(to.xCoord)
            || Double.isNaN(to.yCoord)
            || Double.isNaN(to.zCoord)) {
            return null;
        }
        ...
        int fromZ = MathHelper.floor_double(from.zCoord);
        {
            ...
            if (block.canCollideCheck(world.getBlockMetadata(fromX, fromY, fromZ), false)) {
