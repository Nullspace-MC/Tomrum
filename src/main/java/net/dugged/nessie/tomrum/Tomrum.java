...
import net.dugged.nessie.tomrum.mixins.ISoundHandler;
...
@Mod(modid = "tomrum", version = Tags.VERSION, guiFactory = "net.dugged.nessie.tomrum.GuiConfigFactory")
public class Tomrum {
    ...
    private final KeyBinding reloadAudioEngineKey = new KeyBinding(
        "key.tomrum.reload_audio",
        Keyboard.KEY_B,
        "key.categories.misc");
    ...
    public void onKeyPress(final InputEvent.KeyInputEvent event) {
        ...
        if (this.reloadAudioEngineKey.isPressed()) {
            ((ISoundHandler) Minecraft.getMinecraft()
    ...
    public void onPlayerPreTick(final TickEvent.PlayerTickEvent event) {
        ...
        if (CONFIG.flightInertiaCancellation && player.capabilities.isFlying) {
            ...
            if (!(GameSettings.isKeyDown(settings.keyBindForward) || GameSettings.isKeyDown(settings.keyBindBack)
                || GameSettings.isKeyDown(settings.keyBindLeft)
                || GameSettings.isKeyDown(settings.keyBindRight))) {
    ...
    public void onRenderBlockOverlay(final RenderBlockOverlayEvent event) {
        if (CONFIG.creativeNoclip && event.overlayType == OverlayType.BLOCK
